...
df_alignment_child = pd.read_csv(ALIGNMENT, keep_default_na=False)
...
for session_id in session:
    ...
    # print(session_id, word_val, type(word_val))
    if word_val[0] == '<':
        word_val = '<pause>'
...
for session, features in new_compiled_features.items():
    ...
    output_name = FILENAME_PREFIX + str(session) + '.tsv'
    print(output_name)
    matrix.to_csv(output_name, sep='\t')